...
const MarketTable = () => {
  ...
  useEffect(() => {
    // Mencegah double-fetch di React StrictMode (Dev)
    ...
    const fetchCoins = async () => {
      ...
      try {
        // HANYA FETCH 1 HALAMAN (250 Koin)
        // Ini jauh lebih aman dari rate limit dibanding fetch 1000 koin
        const response = await fetch(
          `https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=250&page=1&sparkline=false`
        );

        if (response.status === 429) {
          throw new Error("Terlalu banyak request. Mohon tunggu sejenak.");
        }
    ...
    <div className="table-container">
      ...
      <div className="filter-header">
        <div className="header-left">
          ...
          <span className="page-info" style={{fontSize: '0.8rem', color: '#00FFA3'}}>
            Showing Top {coins.length > 0 ? coins.length : '...'} Assets
          </span>
        </div>
            ...
            {filteredData.length > 0 ? (
              filteredData.map((coin, index) => (
                ...
                >
                  ...
                  {/* Font Price standar (Fredoka) sesuai request */}
                  <td>
      ...
      <div style={{textAlign: 'center', marginTop: '1rem', color: '#555', fontSize: '0.8rem', paddingBottom: '2rem'}}>
        {loading 
          ...
          : `Displaying ${filteredData.length} of 250 loaded assets.`
        }
      </div>
